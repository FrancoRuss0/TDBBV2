...
@Component
public class EPR_HeaderSql implements MethodInterface {
	@Autowired
	private Utility utility;

	private static final Logger logger = LogManager.getLogger(EPR_HeaderSql.class);
	...
	public void insert(List<Object> flights, Connection connection) throws SQLException, IOException {
		ArrayList<EPR_Header> trasformEPR_Header = new ArrayList<EPR_Header>();
		for (Object flight : flights) {
			trasformEPR_Header.add((EPR_Header) flight);
		}

		PreparedStatement stmt = null;

		// Read the SQL insert query from the file
		String sql = utility.loadSqlFromFile("src/main/resources/query/insert/InsertEpr_header.sql");

		// Create a PreparedStatement to execute the SQL query
		stmt = connection.prepareStatement(sql);

		// Add the flight data to the batch for bulk insertion
		for (EPR_Header EPR_header : trasformEPR_Header) {
			...
			stmt.setObject(12, EPR_header.getLast_Sign_in_Dt(), Types.TIMESTAMP); // Last_Sign_in_Dt (assuming it's a
																					// Date object, convert it to
																					// String)
			...
			stmt.setObject(49, EPR_header.getEffectiveDate(), Types.TIMESTAMP); // EffectiveDate (assuming it's a Date
																				// object, convert it to String)
			...
			stmt.setObject(64, EPR_header.getLast_Sign_in_Dt(), Types.TIMESTAMP); // Last_Sign_in_Dt (assuming it's a
																					// Date object, convert it to
																					// String)
			...
			stmt.setObject(101, EPR_header.getEffectiveDate(), Types.TIMESTAMP); // EffectiveDate (assuming it's a Date
																					// object, convert it to String)
			...
			stmt.setObject(105, utility.nowUtcTimestamp(), Types.TIMESTAMP); // tdb_created_at (it’s not part of
																				// `EPR_header`, assumed it's a
																				// different variable)
			stmt.addBatch();
		}
