...
public class SqlQueries {
	...
	/**
	 * This method processes flight data based on the provided base filename and inserts 
	 * the corresponding records into the database. 
	 * It handles PNR and VCR records, as well as general flight records.
	 * 
	 * @param flights      - List of flight objects to be inserted into the database
	 * @param baseFilename - The name of the file that helps determine the data type (e.g., "Res", "Tkt")
	...
	public void sql(List<Object> flights, String baseFilename, Connection connection) throws Exception {
		try {
			...
			// If the filename does not contain "Res" or "Tkt", handle general flight records
			} else {
				// Get the appropriate SQL interface based on the base filename
				MethodInterface methodInterface = (MethodInterface) context.getBean(baseFilename + "Sql");
