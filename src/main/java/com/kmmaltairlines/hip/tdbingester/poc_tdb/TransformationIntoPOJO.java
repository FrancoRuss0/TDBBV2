...
public class TransformationIntoPOJO {
	...
	/**
	 * Loads a .dat file, processes it using a DataWeave script, and converts it
	 * into a list of POJO objects. The script is dynamically executed based on the
	 * provided input string and filename.
	 * 
	 * @param inputString - The raw input string representing the content of the
	 *                    .dat file.
	 ...
	 * @throws IOException            - If an error occurs while reading the DWL
	 *                                file or performing file operations.
	 * @throws ClassNotFoundException - If the dynamically loaded POJO class is not
	 *                                found.
	 */
	...
	public ArrayList<Object> loadDatFileIntoPOJO(String inputString, String baseFilename)
		...
		// Compile the DWL script, binding the 'payload' and 'filename' variables to the
		// inputString and filename
		DataWeaveScript compile = scriptingEngine.compile(dwlScript, new String[] { "payload", "filename" });

		// Execute the DataWeave script with the provided bindings and capture the
		// result
		...
		// Dynamically construct the class name based on the filename, assuming it's the
		// name of a POJO class
		...
		// Assuming the result of the DataWeave script is a list of objects of the
