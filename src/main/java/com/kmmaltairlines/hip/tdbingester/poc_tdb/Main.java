...
public class Main {
    ...
    // Scheduled task that runs at a fixed interval, based on the configured polling frequency
    ...
    public void scheduledFileProcessing() throws Exception {
        ...
        try {
            ...
            if (blobs.isEmpty()) {
                ...
                //sendErrorEmail("The list of blobs is empty. There are no files to process.", "", "", "", "No blobs to process.");
                        ...
                        for (BlobData encryptedBlob : blobs) {
                            if (encryptedBlob.getName().equals(encFileName)) {
                                try {
                                    ...
                                    byte[] decryptedFile = decrypt.decryptFile(encryptedBlob.getContent().getBytes(), passPhraseChars);
                                    ...
                                    try (InputStream inputStream = Files.newInputStream(tempPath);
                                        ...
                                        if (e.getMessage().equals("File size mismatch between the done file and the decrypted file.")) {
                                            logger.error("File size mismatch for " + doneBlob.getName() + ": " + e.getMessage());
                                            sendErrorEmail(e.getMessage(), doneBlob.getName(), encFileName, "", Arrays.toString(e.getStackTrace()));
                                            moveBlob(doneBlob.getName());
                                        } else {
            ...
            sendErrorEmail(e.getMessage(), "", "", "", Arrays.toString(e.getStackTrace())); // Send error email for database connection issues
        }
    }
