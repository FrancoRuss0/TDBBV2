 ...
 * means that two or more records in the same file contain the exact same values
 * for all the fields which should uniquely identify each record. Take
 * ResRemarks as an example. Each ResRemark record should be uniquely identified
 * by these 4 fields: PNRLocatorID, PNRCreateDate, ResRemarkSeqId,
 * IntraPNRSetNbr. But, it is possible to receive the following two ResRemarks
 * records in the same file:
 *
 * 05|PWMKDF|2017-09-27|2017-11-29 04:35:29.000961|7|SPLIT TO/070822/03NOV17
 * TIUHMS 30/30 29/29 C/29GRPS GREEN|8||||0 05|PWMKDF|2017-09-27|2020-10-07
 * 06:00:00.000000|7|SPLIT TO/070822/03NOV17 TIUHMS 30/30 29/29 C/29GRPS
 * GREEN|8||||0
 * 
 * In this example, the values of the 4 fields that should uniquely identify a
 * ResRemark in a file are the same: - PNRLocatorId: PWMKDF - PNRCreateDate:
 * 2017-09-27 - ResRemarkSeqId: 7 - IntraPNRSetNbr: 0
 * 
 * Trying to insert these two records in the ResRemarks table would result in a
 * "Duplicate Key" database error due to the table's unique constraint.
 * 
 * This class contains two static methods whose job is to remove such duplicate
 * records in PNR and VCR files. Only the record with the latest
 * TransactionDateTime (in case of VCR files) or the latest FromDateTime (in
 * case of PNR files) is kept from the set of duplicates.
 * 
 * These methods work as follows:
 * 
 * 1. Initialize an empty Set of type VCRRecord/PNRRecord called uniqueRecords
 * 
 * 2. Order, in descending order, PNR/VCR records based on PNRLocatorId,
 * PNRCreateDate and FromDateTime (for PNR records) and PrimaryDocNbr,
 * VCRCreateDate and TransactionDateTime (for VCR records). Therefore, in case
 * of duplicate entries, the entry with the latest
 * FromDateTime/TransactionDateTime will occur first in the list.
 * 
 * 3. For each record, check if it is contained in the uniqueRecords Set. A
 * record would already be contained in this set if it is equal (based on the
 * equals() and hashCode() methods on the unique fields) to an existing record
 * in the set.
 * 
...
public class SabreDuplicateFileRecordPruner {
	...
	public static Set<? extends VCRRecord> removeDuplicateVCRRecords(List<VCRRecord> vcrRecords) {
		...
		vcrRecords.stream().sorted(new Comparator<VCRRecord>() {
			@Override
			public int compare(VCRRecord lhs, VCRRecord rhs) {
				...
				if (lhs.getPrimaryDocNbr().compareTo(rhs.getPrimaryDocNbr()) == 0) {
					if (lhs.getVCRCreateDate().compareTo(rhs.getVCRCreateDate()) == 0) {
						...
						return DESCENDING * (lhs.getVCRCreateDate().compareTo(rhs.getVCRCreateDate()));
				} else
					return DESCENDING * (lhs.getPrimaryDocNbr().compareTo(rhs.getPrimaryDocNbr()));
			}
	...
	/*
	 * Removes duplicate PNR records based on the equals() method defined in each
