...
import com.airmalta.hip.tdbingester.exceptions.EncFileDoesNotExistException;
import com.airmalta.hip.tdbingester.filepojos.DoneFile;
import com.google.common.io.CountingInputStream;

/**
 * 
 * This class programmatically uses the SFTP endpoint in the main flow to read the .enc file from the SFTP corresponding the the 
 * current .done file stored in the doneFile flow variable.
 * 
...
public class QuerySingleFileProcessor{
	...
	/**
	 * Reads DoneFile POJO from doneFile flowVariable, gets the corresponding .enc file name using getEncFileName(), 
	 * uses programmatically the SFTP endpoint in the main flow to read the .enc file and, if found, returns it
	 * as a {@link com.google.common.io.CountingInputStream}. A CountingInputStream is needed in order to validate that
	 * the byte size of the .enc file is equal to the byte size denoted in the corresponding .done file.
	 */
...
//		
    ...
    /**
     * On startup, get the necessary details to be able to retrieve files from the SFTP endpoint in the main flow programmatically  
     * through the onCall method
     */
//    @Override
