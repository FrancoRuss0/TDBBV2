...
import com.airmalta.hip.tdbingester.exceptions.UnknownFileTypeException;
...
public class DoneFile {
	...
	public static final Pattern DAT_FILE_NAME_PATTERN = Pattern.compile("^(.*)_(\\d{8})(_SPECIAL)?\\.dat$", Pattern.CASE_INSENSITIVE);
	public static final Pattern ENC_FILE_NAME_PATTERN = Pattern.compile("^km_(.*)_(\\d{8})(_SPECIAL)?_dat\\.enc$", Pattern.CASE_INSENSITIVE);
	public static final Pattern GPG_FILE_NAME_PATTERN = Pattern.compile("^km_(.*)_(\\d{8})(_SPECIAL)?_dat\\.zip.gpg$", Pattern.CASE_INSENSITIVE);
	...
	/**
	 * A utility method for other classes to parse .done file name without creating a DoneFile POJO
	 * 
	 * @param filename - a string obeying DATETIME_REGEX_PATTERN pattern and the 8 numbers in the pattern follow the DONEFILE_DATE_PATTERN date format
	...
	/**
	 * Construct .enc file name by using the .done file name without extension and apending the ".enc" file extension.
	 * @return .enc file name, with ".enc" extension
	 */
	public String getEncFileName() {
		/*if (this.getFileNameWithoutExtension().toLowerCase().startsWith("km_epr") || this.getFileNameWithoutExtension().toLowerCase().startsWith("km_ssi") || this.getFileNameWithoutExtension().toLowerCase().startsWith("km_batch")) {
			return this.getFileNameWithoutExtension() + ".zip.gpg";
		}
		else {
			return this.getFileNameWithoutExtension() + ".enc";
		}*/
		return this.getFileNameWithoutExtension() + ".zip.gpg";
	}
	 ...
	 * 
	 * @param filename 
	 * @return Returns true if filename is contained in the list of files specified in the .done file.
	 */
	 ...
	 * @return True whenever the .done file contains all the filenames in the given collection, otherwise False.
	 */
	...
	/**
	 * Sets the filename and the timestamp of this .done file. Timestamp is extracted using the {@link getTimestampFromDoneFilename} method.
	 * @param fileName
	 */
	 ...
	 * @return A string of 8 numeric characters representing the date in the file name.
	 */
	...
	public enum EncFileType {
		...
		public static EncFileType fromEncFileName(String fileName) throws UnknownFileTypeException {
			...
			throw new UnknownFileTypeException(String.format("Cannot determine file type from file name '%s'.", fileName));
		}
	}
