...
public class TransformationLoadDatFileIntoPOJO {

	public ArrayList<Object> LoadDatFileIntoPOJO(String inputString, String filename) throws IOException, ClassNotFoundException {
		...
		// Esegui lo script
        DataWeaveResult result = compile.write(new ScriptingBindings().addBinding("payload", inputString, "text/plain", new HashMap<>()).addBinding("filename", filename, "text/plain", new HashMap<>()));
        
        String className = "com.airmalta.hip.tdbingester.filepojos." + filename;
        
     // Carica la classe dinamicamente
        Class<?> clazz = Class.forName(className);

        // Ora, supponiamo che il risultato DataWeave sia una lista di oggetti di tipo 'clazz'
